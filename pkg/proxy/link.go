...
func NewToxicLink(proxy *Proxy, collection *ToxicCollection, direction stream.Direction) *ToxicLink {
	link := &ToxicLink{
		stubs:     make([]*toxics.Stub, len(collection.chain[direction]), cap(collection.chain[direction])),
		proxy:     proxy,
		toxics:    collection,
		direction: direction,
	}
	...
	for i := 0; i < len(link.stubs); i++ {
		...
		link.stubs[i] = toxics.NewStub(last, next)
		last = next
	}
...
func (link *ToxicLink) Start(name string, source io.Reader, dest io.WriteCloser) {
	...
	for i, toxic := range link.toxics.chain[link.direction] {
		if stateful, ok := toxic.Toxic.(toxics.Stateful); ok {
...
func (link *ToxicLink) AddToxic(toxic *toxics.Wrapper) {
	i := len(link.stubs)

	newin := make(chan *stream.Chunk, toxic.BufferSize)
	link.stubs = append(link.stubs, toxics.NewStub(newin, link.stubs[i-1].Output))
...
func (link *ToxicLink) UpdateToxic(toxic *toxics.Wrapper) {
...
func (link *ToxicLink) RemoveToxic(toxic *toxics.Wrapper) {
	i := toxic.Index

	if link.stubs[i].InterruptToxic() {
		cleanup, ok := toxic.Toxic.(toxics.Cleanup)
		if ok {

...
func NewServer() *ApiServer {
	return &ApiServer{
		Collection: proxy.NewCollection(),
	}
}
...
func (server *ApiServer) ResetState(response http.ResponseWriter, request *http.Request) {
	...
	for _, p := range proxies {
		err := p.Start()
		if err != proxy.ErrProxyAlreadyStarted && apiError(response, err) {
			return
		}
...
func (server *ApiServer) ProxyCreate(response http.ResponseWriter, request *http.Request) {
	...
	input := proxy.Proxy{Enabled: true}
	err := json.NewDecoder(request.Body).Decode(&input)
	if apiError(response, errors.JoinError(err, errors.ErrBadRequestBody)) {
		return
	}

	if len(input.Name) < 1 {
		apiError(response, errors.JoinError(fmt.Errorf("name"), errors.ErrMissingField))
		return
	}
	if len(input.Upstream) < 1 {
		apiError(response, errors.JoinError(fmt.Errorf("upstream"), errors.ErrMissingField))
		return
	}
...
func (server *ApiServer) Populate(response http.ResponseWriter, request *http.Request) {
	...
	data, err := json.Marshal(struct {
		*errors.ApiError `json:",omitempty"`
...
func (server *ApiServer) ProxyUpdate(response http.ResponseWriter, request *http.Request) {
	...
	p, err := server.Collection.Get(vars["proxy"])
	if apiError(response, err) {
		return
	}

	// Default fields are the same as existing proxy
	input := proxy.Proxy{Listen: p.Listen, Upstream: p.Upstream, Enabled: p.Enabled}
	err = json.NewDecoder(request.Body).Decode(&input)
	if apiError(response, errors.JoinError(err, errors.ErrBadRequestBody)) {
		return
	}
	...
	data, err := json.Marshal(proxyWithToxics(p))
	if apiError(response, err) {
		return
	}
...
func (server *ApiServer) ToxicShow(response http.ResponseWriter, request *http.Request) {
	...
	if toxic == nil {
		apiError(response, errors.ErrToxicNotFound)
		return
	}
...
func (server *ApiServer) Version(response http.ResponseWriter, request *http.Request) {
	...
	_, err := response.Write([]byte(proxy.Version))
	if err != nil {
...
func proxyWithToxics(p *proxy.Proxy) (result proxyToxics) {
	result.Proxy = p
	result.Toxics = p.Toxics.GetToxicArray()
	return
}

func proxiesWithToxics(proxies []*proxy.Proxy) (result []proxyToxics) {
	for _, p := range proxies {
		result = append(result, proxyWithToxics(p))
	}
	return
}

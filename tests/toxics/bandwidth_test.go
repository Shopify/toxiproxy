...
func TestBandwidthToxic(t *testing.T) {
	...
	rate := 1000 // 1MB/s
	proxy.Toxics.AddToxicJson(ToxicToJson(t, "", "bandwidth", "upstream", &toxics.Bandwidth{Rate: int64(rate)}))
...
func BenchmarkBandwidthToxic100MB(b *testing.B) {
	...
	proxy.Toxics.AddToxicJson(ToxicToJson(nil, "", "bandwidth", "upstream", &toxics.Bandwidth{Rate: 100 * 1000}))
